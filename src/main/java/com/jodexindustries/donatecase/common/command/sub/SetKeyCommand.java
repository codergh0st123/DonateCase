   ...
   public boolean execute(@NotNull DCCommandSender sender, @NotNull String label, String[] args) {
      if (args.length < 3) {
         ...
         if (!DCTools.isValidPlayerName(playerName)) {
            sender.sendMessage(DCTools.prefix(DCTools.rt(this.api.getConfigManager().getMessages().getString("player-not-found"), LocalPlaceholder.of("%player%", playerName))));
            return true;
         } else {
            int keys;
            try {
            ...
            CaseData data = this.api.getCaseManager().get(caseName);
            if (data != null) {
               this.api.getCaseKeyManager().set(caseName, playerName, keys).thenAcceptAsync((status) -> {
                  ...
                  if (status == DatabaseStatus.COMPLETE) {
                     sender.sendMessage(DCTools.prefix(DCTools.rt(this.api.getConfigManager().getMessages().getString("keys-sets"), placeholders)));
                     if (args.length < 4 || !args[3].equalsIgnoreCase("-s")) {
                        DCPlayer target = this.api.getPlatform().getPlayer(playerName);
                        if (target != null) {
                           target.sendMessage(DCTools.prefix(DCTools.rt(this.api.getConfigManager().getMessages().getString("keys-sets-target"), placeholders)));
                        }
                     }
                  }
