...
public class CasesCommand extends DefaultCommand {
   ...
   public boolean execute(@NotNull DCCommandSender sender, @NotNull String label, String[] args) {
      ...
      for(CaseData data : this.api.getCaseManager().getMap().values()) {
         ++num;
         Collection<LocalPlaceholder> placeholders = LocalPlaceholder.of(data);
         placeholders.add(LocalPlaceholder.of("%num%", num));
         sender.sendMessage(DCTools.prefix(DCTools.rt(this.api.getConfigManager().getMessages().getString("list-of-cases"), placeholders)));
      }
