...
public class HISTORYItemHandlerImpl implements TypedItemHandler {
   public CaseGui.@NotNull Item handle(@NotNull CaseGuiWrapper caseGui, CaseGui.@NotNull Item item) throws TypedItemException {
      ...
      boolean handled = this.handleHistoryItem(caseData, item, caseGui.getGlobalHistoryData());
      if (!handled) {
         try {
            ConfigurationNode notFoundNode = item.node().node(new Object[]{"HistoryNotFound"});
            if (!notFoundNode.isNull()) {
               item.material((CaseDataMaterial)notFoundNode.get(CaseDataMaterial.class));
            } else {
   ...
   private boolean handleHistoryItem(CaseData caseData, CaseGui.Item item, List<CaseData.History> globalHistoryData) {
      ...
      if (historyCaseData == null && !isGlobal) {
         ...
         CaseData.History data = this.getHistoryData(caseType, isGlobal, globalHistoryData, index);
         if (data == null) {
            return false;
         } else {
            if (isGlobal) {
            ...
            if (historyCaseData == null) {
               ...
               if (historyItem == null) {
                  ...
                  Collection<LocalPlaceholder> placeholders = this.getPlaceholders(historyCaseData, data, historyItem);
   ...
   private static String getActionDisplayName(String action, String groupDisplayName, Map<String, CaseDataItem.RandomAction> randomActions) {
      if (action != null && !action.isEmpty()) {
         String displayName = ((CaseDataItem.RandomAction)randomActions.getOrDefault(action, new CaseDataItem.RandomAction())).displayName();
